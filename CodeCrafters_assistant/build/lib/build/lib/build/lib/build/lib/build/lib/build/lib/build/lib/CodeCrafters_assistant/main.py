...
class InputManager(IdFormat, Translate):
    def __init__(self):
        ...
        if system_language[0] in self.languages.values():
            tree = ET.parse(f"CodeCrafters_assistant\\CodeCrafters_assistant\\localization_{system_language[0]}.xml")
        else:
            tree = ET.parse("CodeCrafters_assistant\\CodeCrafters_assistant\\localization_en.xml")
